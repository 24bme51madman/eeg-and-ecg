...
export const EcgWaveformTrace: FC<EcgWaveformTraceProps> = ({
  ...
  // Live readings state
  const [currentMv, setCurrentMv] = useState<number>(0.02);
  ...
  const [stDeviationMv, setStDeviationMv] = useState<number>(0.01);
  ...
  }, [isSimulated, patientModeId, activePreset]);
    ...
    <div className="w-full">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-red-200/80 pb-2.5 text-xs">
          ...
          >
            <Heart
              className={`h-3.5 w-3.5 fill-current transition-transform duration-100 ${
                isBeating ? 'scale-125 text-white' : 'text-red-600'
              }`}
            />
        ...
        <div className="flex flex-wrap items-center gap-2 font-mono text-[11px]">
          ...
          <div className="border border-red-200 bg-white px-2 py-0.5 text-neutral-800">
            R-R: <span className="font-semibold text-[#141517]">{rrIntervalMs} ms</span>
          </div>

          <div className="border border-red-200 bg-white px-2 py-0.5 text-neutral-800">
            HRV (RMSSD):{' '}
            <span className="font-semibold text-red-800">{hrvRmssdMs} ms</span>
          </div>
        ...
        <div className="absolute top-2 right-3 rounded-none border border-red-300/80 bg-white/95 px-2 py-0.5 font-mono text-[10px] text-red-900 flex items-center gap-1.5 shadow-2xs">
          <span
            className={`h-1.5 w-1.5 rounded-full ${
              isBeating ? 'bg-red-600 scale-125' : 'bg-emerald-600'
            } transition-all`}
          />
          <span className="font-semibold">{cardiacSpecs.rhythm.split('(')[0].trim().toUpperCase()}</span>
        </div>
      </div>
      ...
      <div className="mt-3.5 grid grid-cols-2 gap-2 border-t border-red-200/70 pt-3 sm:grid-cols-4 lg:grid-cols-4">
        ...
        <div className="border border-red-200/90 bg-white p-2.5 shadow-2xs">
          <div className="flex items-center justify-between font-mono text-[10px] text-neutral-500">
            ...
            <Heart className={`h-2.5 w-2.5 ${isBeating ? 'text-red-600 fill-current' : 'text-neutral-400'}`} />
          </div>
          <div className="mt-1 flex items-baseline justify-between font-mono">
            ...
            >
              {heartRate > 100 ? 'TACHY' : heartRate < 60 ? 'BRADY' : 'EUCARDIA'}
            </span>
          </div>
        ...
        <div className="border border-red-200/90 bg-white p-2.5 shadow-2xs">
          ...
          <div className="mt-1 flex items-baseline justify-between font-mono">
            ...
            <span className="text-[10px] font-mono text-neutral-500">BEAT-TO-BEAT</span>
          </div>
          <div className="mt-1 text-[10px] font-mono text-neutral-500">
            REF: 600–1200 ms
          </div>
        ...
        <div className="border border-red-200/90 bg-white p-2.5 shadow-2xs">
          ...
          <div className="mt-1 flex items-baseline justify-between font-mono">
            ...
            <span
              className={`text-[10px] font-semibold ${
                hrvRmssdMs > 50 ? 'text-emerald-700' : 'text-amber-700'
              }`}
            >
              {hrvRmssdMs > 50 ? 'HIGH VAGAL' : 'SYMPATHETIC'}
            </span>
          </div>
          <div className="mt-1 text-[10px] font-mono text-neutral-500">
            VAGAL AUTONOMIC TONE
          </div>
        </div>
        ...
        <div className="border border-red-200/90 bg-white p-2.5 shadow-2xs">
          ...
          <div className="mt-1 flex items-baseline justify-between font-mono">
            ...
            <span className="text-[10px] text-emerald-700 font-semibold">NARROW (&lt;100)</span>
          </div>
          <div className="mt-1 text-[10px] font-mono text-neutral-500">
        ...
        <div className="border border-red-200/90 bg-white p-2.5 shadow-2xs">
          ...
          <div className="mt-1 flex items-baseline justify-between font-mono">
            ...
            <span className="text-[10px] text-neutral-600">AV CONDUCTION</span>
          </div>
          <div className="mt-1 text-[10px] font-mono text-neutral-500">
            REF: 120–200 ms
          </div>
        ...
        <div className="border border-red-200/90 bg-white p-2.5 shadow-2xs">
          ...
          <div className="mt-1 flex items-baseline justify-between font-mono">
            ...
            <span className="text-[10px] text-emerald-700">NORMAL (&lt;440)</span>
          </div>
          <div className="mt-1 text-[10px] font-mono text-neutral-500">
            REPOLARIZATION
          </div>
        ...
        <div className="border border-red-200/90 bg-white p-2.5 shadow-2xs">
          ...
          <div className="mt-1 flex items-baseline justify-between font-mono">
            ...
            <span className="text-[10px] text-emerald-700 font-semibold">
              {coherenceScore > 0.8 ? 'SYNCHRONIZED' : 'DECOUPLED'}
            </span>
          </div>
          <div className="mt-1 text-[10px] font-mono text-neutral-500">
            THETA ↔ R-R COUPLING
      ...
      <div className="mt-2.5 border border-red-200 bg-red-50/60 p-2.5 font-mono text-xs text-neutral-800 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <span className="text-neutral-800">{cardiacSpecs.rhythm}</span>
          </div>
        </div>
        <div className="text-[11px] text-neutral-600 sm:text-right">
          <span className="font-medium text-neutral-800">AUTONOMIC PROFILE:</span> {cardiacSpecs.autonomicTone}
        </div>
      </div>
    </div>
  );
